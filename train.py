...
import opts
import models
from dataloader_raml import *
import skimage.io
import eval_utils
import misc.utils as utils
from misc.rewards import init_scorer, get_self_critical_reward
from misc.loss_wrapper import LossWrapper_soft
...
def train(opt):
    ...
    lw_model = LossWrapper_soft(model, opt)
    ...
    try:
        while True:
            ...
            model_out = dp_lw_model(fc_feats, att_feats, labels, masks, att_masks,
                                    data['gts'], torch.arange(0, len(data['gts'] if len(data['gts']) == 1 else data['gts'][0])), sc_flag)
