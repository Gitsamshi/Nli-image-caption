...
class LanguageModelCriterionPerExample(nn.Module):
    '''
    return log loss per example (Bz, )
    '''
    def __init__(self):
        super(LanguageModelCriterionPerExample, self).__init__()
    ...
    def forward(self, input, target, mask):
        ...
        output = -input.gather(2, target.unsqueeze(2)).squeeze(2) * mask
        output = torch.sum(output, dim=1) / torch.sum(mask, dim=1)
...
class ReduceLROnPlateau(object):
    ...
    def __init__(self, optimizer, mode='min', factor=0.1, patience=10, verbose=False, threshold=0.0001, threshold_mode='rel', cooldown=0, min_lr=0, eps=1e-08):
        self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode=mode, factor=factor, patience=patience, verbose=verbose, threshold=threshold, threshold_mode=threshold_mode, cooldown=cooldown, min_lr=min_lr, eps=eps)
        self.optimizer = optimizer
